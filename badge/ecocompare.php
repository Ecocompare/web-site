...
function vueEcocompare(id, base, prod){	
	...
	<? if(isset($_GET["ecocompare_personnaliser"])&&isset($_GET["nbbandeaux"])){  ?>
		var ecocompare_personnaliser = true;
		new_couleur = '<?= $_GET["ecocompare_personnaliser"]; ?>';
	<?	}else{ ?>
		var ecocompare_personnaliser = false;
	...
	<?	}  ?>
		...
		mydiv.innerHTML += contenu;
				...
				if(personnalisation===true){
					...
					document.getElementById("badge_bandeau_sup2").style.background= "#"+new_couleur;
					document.getElementById("ecocompare_bandeau_sup").style.background= "#"+new_couleur;

					if(<?= $_GET["nbbandeaux"] ?> == "2"){
						document.getElementById("badge_bandeau_inf").style.background= "#"+new_couleur;
						
						var $r = parseInt( new_couleur.substr(0,2), 16);
						var $g = parseInt( new_couleur.substr(2,2), 16);
						var $b = parseInt( new_couleur.substr(3,2), 16);
						if($r + $g + $b > 382){
