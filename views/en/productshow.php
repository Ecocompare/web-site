...
<div class="badge">
<img width="150" alt="<?=stripslashes($this->product['name'])?>" src="bilan-produit-eco-<?=toUrl($this->product['name'])?>-<?=$this->product['id']?>-<?=$this->lang?>.png"/>
...
<div class="productfeatures">
		...
		<? if ($this->company != null){ ?>
			<strong><a href="<?=$GLOBALS['base']?><?=toURL($this->company['title'])?>_c<?=$this->company['id']?>.html">See the <?=$this->company['title']?> page</a></strong><br/>
		<? } ?>
...
<div class="productdescription tooltip2">
	...
	<div class="faded">
		...
		<br/>Add on <? echo $this->formatTime($this->product['pubdate'])?><br/>
		<? } ?>
	</div>
...
<div id="productpanes">
	...
	<div class="hometabs">
		<a id="tab1" class="on" href="#1"><img src="<?=$GLOBALS['base']?>style/rating.png" alt="Our opinion"/>&nbsp;&nbsp;Our opinion</a>
	</div>
	...
	<!--	
				...
				<div class="ratingdetail">
					...
					<?php $this->displayLifeCycleCompact($this->product['id'],$this->lang); ?>
				</div>
			</div>
				...
				<div class="ratingdetail">
					...
					<?php $this->displayThemescore($this->product['id'],'3,4,5,6,7',$this->lang); ?>
				</div>
				...
				<div class="ratingdetail">
					...
					<?php $this->displayRatingDetail($this->product['id'],'2',0,$this->lang); ?>
...
<?}?>
	...
	<? if (count($this->relatedProducts) != 0){ ?>
		<div class="hometabs">
			<a id="tab3" class="on" href="#1"><img src="<?=$GLOBALS['base']?>style/folder.png" alt="Similar products"/>&nbsp;&nbsp;Similar products </a>
		</div>
	...
	<div class="hometabs">
		<a id="tab2" class="on" href="#1"><img src="<?=$GLOBALS['base']?>style/avis-produit-ecologique.png" alt="Guest reviews <?=$this->product['name']?>"/>&nbsp;&nbsp;Guest reviews (<?=count($this->comments)?>)</a>
	</div>
	...
	<div class="comments" id="section2">
		...
		<strong>Please give your opinion about this product</strong>
		<form id="form" action="<?=$GLOBALS['base']?>comments/add" method="post">
			...
			<label for="name" style="display:none">Pseudo</label><input name="name" id="name"/> Your name or pseudo (required)<br/>
			<label for="email" style="display:none">Email</label><input name="email" id="useremail"/> Your email (required, invisible for other users)<br/>
			<label for="captcha" style="display:none">Enter that code (anti-spam control)</label>
			<input name="TzUio23bc" id="captcha"/> Enter that code (anti-spam control) : <br/>
		<img src="<?=$GLOBALS['base']?>/captcha/captcha.php" alt="code captcha"/><br/>
			<label for="text" style="display:none">Comment</label><textarea name="text" id="text"></textarea><br/>
